...
def write_lega_nazionale_on_brasileiro_format(gameFilePath,year):
    ...
    with open(abspath('data/italiano/simples'+year),'w') as filestream:
        filestream.write("[")
...
if __name__ == "__main__":
    ...
    folder = abspath(configreader.parser.get('folder italiano','folder'))
    for fil in os.listdir(folder):
        if fil.startswith("lega-nazionale-") :
            path = folder + os.sep + fil
            write_lega_nazionale_on_brasileiro_format(path,path[-8:-4])
    pass