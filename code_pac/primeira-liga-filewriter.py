    ...
    for line in open_csv:
        ...
        if row[0] == "P1":
            ...
            if row[result_index] == 'D':
                ...
                score = 3
                round_[player_index[player]][1] = round_[player_index[player]][1] + int(score)
            if count == int(number_of_games_per_round) :
                count = 0
...
def write_primeira_liga_on_brasileiro_format(gameFilePath,year):
    ...
    with open(abspath('data/portugal/simples'+year),'w') as filestream:
        filestream.write("[")
...
if __name__ == "__main__":
    ...
    folder = abspath(configreader.parser.get('folder portugal','folder'))
    for fil in os.listdir(folder):
        if fil.startswith("liga-1-") :
            path = folder + os.sep + fil
            write_primeira_liga_on_brasileiro_format(path,path[-8:-4])
    pass