...
from GameQualityAssessment.code_pac.model import LegaNazionaleGame
import GameQualityAssessment.code_pac.italiano.model as model
...
import GameQualityAssessment.code_pac.plots.italianoPlots as itp
...
class FootballView(wx.Frame):
    ...
    def __setData(self,event):
        ...
        leadChgs = []
        for game in games :
            lngame = LegaNazionaleGame(game)
            dramas.append(Drama(game=lngame,ignored=0).getMeasureValue())
            uncerts.append(Uncertainty(game=lngame,ignored=0).getMeasureValue())
            leadChgs.append(LeadChange(game=lngame,ignored=0).getMeasureValue())
        ...
        if len(dramas) > 1:
            ...
            itp.values_histogram(dramas,panel_drama,'Drama')
            itp.values_histogram(uncerts,panel_uncert,'Uncertainty')
            itp.values_histogram(leadChgs,panel_leadChg,'Lead Change')
            itp.values_histogram(overall_evals,panel_evals,'Overall Eval.')
            ...
            itp.points(games[0],panel_points)
            itp.positions(games[0],panel_positions)
