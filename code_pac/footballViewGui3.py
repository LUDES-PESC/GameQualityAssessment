...
from GameQualityAssessment.code_pac.model import LaLigaGame
import GameQualityAssessment.code_pac.espanhol.model as model
...
import GameQualityAssessment.code_pac.plots.espanholPlots as ep
...
class FootballView(wx.Frame):
    ...
    def __setData(self,event):
        ...
        leadChgs = []
        for game in games :
            llgame = LaLigaGame(game)
            dramas.append(Drama(game=llgame,ignored=0).getMeasureValue())
            uncerts.append(Uncertainty(game=llgame,ignored=0).getMeasureValue())
            leadChgs.append(LeadChange(game=llgame,ignored=0).getMeasureValue())
        ...
        if len(dramas) > 1:
            ...
            ep.values_histogram(dramas,panel_drama,'Drama')
            ep.values_histogram(uncerts,panel_uncert,'Uncertainty')
            ep.values_histogram(leadChgs,panel_leadChg,'Lead Change')
            ep.values_histogram(overall_evals,panel_evals,'Overall Eval.')
            ...
            ep.points(games[0],panel_points)
            ep.positions(games[0],panel_positions)
