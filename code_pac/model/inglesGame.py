...
import GameQualityAssessment.code_pac.ingles.model.game as ENmodel

class PremierLeagueGame(GenericGame):
    def __init__(self, game):
        if not isinstance(game, ENmodel.Game):
            raise TypeError('Arg must to be a brasileiro.model.Game instance')
        self._winner = None
        super(PremierLeagueGame, self).__init__(game)
...
if __name__ == "__main__":
    ...
    the_list = ENmodel.Game.retrieveList()
    values=[]
    for game in the_list:
        g = PremierLeagueGame(game)
